...
def main():
    ...
    name =  cfg.objective.loss_type + '_' + cfg.tree.tree_type
    ...
    if cfg.data.empFreq:
        raise NotImplementedError('KL divergence computation is not supported yet.')
    else:
        emp_tree_freq = None
    
    model = ParsiVBPI(taxa, data, emp_tree_freq=emp_tree_freq, cfg=cfg).to(cfg.base.device)
